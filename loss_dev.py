...
from utils.general import bbox_iou, bbox_alpha_iou, box_iou, box_giou, box_diou, box_ciou, xywh2xyxy # 1221 bbox_iou 用於計算loss
...
class ComputeLossOTA: # 1129 走這
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        loss = lbox + lobj + lcls # loss 由3部分駔成 邊界框形狀 物件是否存在 與 類別
        return loss * bs, torch.cat((lbox, lobj, lcls, loss)).detach()
    ...
    def build_targets(self, p, targets, imgs):
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            from_which_layer = from_which_layer[fg_mask_inboxes.to('cpu')] # 1027 偵錯 1129 更新
...
if __name__ == '__main__':
    ...
    box1 = torch.load('box1.pt') # x1 y1 x2 y2 pred
    box2 = torch.load('box2.pt') # x1 y1 s2 y2 ground truth
    print(box1)
    print(box2)
    print(box1.shape)
    print(box2.shape)
    
    box1 = torch.tensor([[0.25], [0.25], [0.75], [0.75]]) # shape(4, 1)
    box2 = box1.T # shape(1, 4)
    print(box1.shape)
    print(box2.shape)
    
    lbox = torch.zeros(1, device='cuda:0')
                       
    iou = bbox_iou(box1, box2, x1y1x2y2=False, CIoU=True)  # iou(prediction, target) 預設使用CIoU 方式計算
    lbox += (1.0 - iou).mean()  # iou loss
    print(lbox)

    print(torch.acos(torch.tensor(0)))
