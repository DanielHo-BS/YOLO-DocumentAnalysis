...
def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False, WIoU=False, EIoU=False, FEIoU=False, eps=1e-7):
    ...
    if GIoU or DIoU or CIoU or WIoU or EIoU or FEIoU:
        cw = torch.max(b1_x2, b2_x2) - torch.min(b1_x1, b2_x1)  # convex (smallest enclosing box) width
        ch = torch.max(b1_y2, b2_y2) - torch.min(b1_y1, b2_y1)  # convex height
        if CIoU or DIoU or EIoU or FEIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            else: # 0221 Focal-EIoU https://arxiv.org/abs/2101.08158
                w_dis=torch.pow(b1_x2-b1_x1-b2_x2+b2_x1, 2)
                h_dis=torch.pow(b1_y2-b1_y1-b2_y2+b2_y1, 2)
                cw2=torch.pow(cw , 2)+eps
                ch2=torch.pow(ch , 2)+eps
                gamma = 0.5
                return (iou**gamma)*(1.0-(iou-(rho2/c2+w_dis/cw2+h_dis/ch2)))-1.0
...
def non_max_suppression_alt(prediction, conf_thres=0.25, iou_thres=0.45, ioa_thres = 1, classes=None, agnostic=False, multi_label=False,
    ...
    """
    nc = prediction.shape[2] - 5  # number of classes
    xc = prediction[..., 4] > 0.1  # candidates
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if nc == 1:
            ...
            x[:, 5:] *= x[:, 4:5]  # conf = obj_conf * cls_conf
            #x[:, 9] = torch.clamp(x[:, 9]*2, min=0, max=0.99) # 1227 降低頁碼被漏檢的機率 頁碼信心*2 簡單快速
...
def box_ioa(box1, box2):
    ...
    # inter(N,M) = (rb(N,M,2) - lt(N,M,2)).clamp(0).prod(2)
    inter = (torch.min(box1[:, None, 2:], box2[:, 2:]) - torch.max(box1[:, None, :2], box2[:, :2])).clamp(0).prod(2)
    inter_without_self = ~torch.eye(len(inter), dtype = torch.bool).to(inter.device) * inter # 移除自己的重複框

    return inter_without_self / area1 # ioa = inter / min(area1, area2)
#=======================non_max_suppression_alt=======================#
...
def nms(boxes, scores, iou_threshold):
    ...
    eps=1e-6
    idxs = ((torch.round(scores) + eps) * box_area(boxes) * torch.sum(box_iou(boxes, boxes) > 0.85, dim = 0)).argsort() # 面積由小到大的 索引 1227 修改
