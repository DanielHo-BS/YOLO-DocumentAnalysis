...
##### end of PRB-FPN #####  # 1115 實作 PRB-net
...
class CBAM(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):
        ...
        self.channel_attention = ChannelAttentionModule(c2)
...
##### end of CBAM #####  # 0103 實作 CBAM

#### WeightedSum for RFCR #####  # 0110 實作 RFCR 寫死只有4個
class WeightedSum(nn.Module):
    def __init__(self, init_value=0.25):
        super(WeightedSum, self).__init__()
        self.weight0 = nn.Parameter(torch.FloatTensor([init_value]))
        self.weight1 = nn.Parameter(torch.FloatTensor([init_value]))
        self.weight2 = nn.Parameter(torch.FloatTensor([init_value]))
        self.weight3 = nn.Parameter(torch.FloatTensor([init_value]))
    def forward(self, x):
        x = x[0]*self.weight0 + x[1]*self.weight1 + x[2]*self.weight2 + x[3]*self.weight3
        return x
##### end of WeightedSum for RFCR #####  # 0110 實作 RFCR
    
#### MBConv for RFCR #####  # 0110 實作 RFCR 中的 MBConv
