...
class WindowAttention_v2(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, attn_drop=0., proj_drop=0.,
        ...
        self.logit_scale = nn.Parameter(torch.log(10 * torch.ones((num_heads, 1, 1))), requires_grad=True).to('cpu')
        ...
        self.qkv = nn.Linear(dim, dim * 3, bias=False, device='cpu')
        if qkv_bias:
            ...
            self.v_bias = None
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Linear(dim, dim, device='cpu')
        self.proj_drop = nn.Dropout(proj_drop)
        self.softmax = nn.Softmax(dim=-1)
...
class ST2CSPC(nn.Module):
    ...
    def forward(self, x):
        y1 = self.cv3(self.m(self.cv1(x))).to('cpu')
        y2 = self.cv2(x).to('cpu')
        return self.cv4(torch.cat((y1, y2), dim=1))
