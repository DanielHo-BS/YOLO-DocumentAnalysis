...
class Indicator():
    ...
    def visualize(self,index = 0,filename = None,save = False): #temp.draw(index,filename)
        ...
        if save:
            ...
            cv2.imencode('.jpg', image)[1].tofile('./out/' + os.path.basename(filename)[:-4] + image_type + '_visualize.jpg') #中文
...
def main(image_folder,ground_truth_folder,prediction_folder):
    ...
    #print(len(temp))
    #print(temp[0])
    temp.visualize(filename = '5656765_2.jpg',save = True)
...
if __name__ in '__main__':
    ground_truth_folder = "../datasets/old/hand_craft_v10/labels/val"
    prediction_folder = "./runs/detect/0223/exp2/labels"
    image_folder = "../datasets/old/hand_craft_v10/images/val"
    os.makedirs('./out',exist_ok = True)
