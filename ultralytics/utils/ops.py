...
from .metrics import box_iou, box_ioa
...
def non_max_suppression_soft(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        boxes, scores = x[:, :4] + c, x[:, 4]  # boxes (offset by class), scores
        #i = torchvision.ops.nms(boxes, scores, iou_thres)  # NMS
        i = soft_nms(boxes, scores, iou_thresh=iou_thres, score_threshold=conf_thres)
        i = i[:max_det]  # limit detections
        ...
        # Using IoA for the removal of small objects in big objects
        ioa = box_ioa((boxes-c)[i], (boxes-c)[i]) > ioa_thres   # ioa matrix
        i = i[ioa.sum(0) < 2]
